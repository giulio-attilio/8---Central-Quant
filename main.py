...
# Versão: 2026-06-17-DONKEY-H4-ONLY-WATCHDOG-CORRIGIDO
#
# Lógica:
...
def montar_resumo_mensal():
    ...
    trades_donkey_mes = [t for t in do_mes if is_donkey_trade_event(t)]
    gestao_dupla = calcular_metricas_gestao_dupla_donkey(trades_donkey_mes)
    ...
    msg = (
        ...
        f"🎯 Gestão dupla Donkey 50/100\n"
        f"SL50 acionados: {len(gestao_dupla['sl50s'])}\n"
        f"SL100 acionados: {len(gestao_dupla['sl100s'])}\n"
        f"Resultado médio SL50:\n{fmt_pct(gestao_dupla['media_sl50'])}\n"
        f"Resultado médio SL100:\n{fmt_pct(gestao_dupla['media_sl100'])}\n"
        f"Extensão média pós-TP50:\n{fmt_pct(gestao_dupla['media_extensao'])}\n"
        f"Resultado final médio da gestão dupla:\n{fmt_pct(gestao_dupla['media_final_gestao_dupla'])}\n"
        f"Melhor extensão pós-TP50:\n{gestao_dupla['melhor_extensao']}\n"
        f"Maior devolução pós-TP50:\n{gestao_dupla['maior_devolucao']}\n\n"
        f"PnL realizado:\n"
        f"{fmt_pct(pnl_total)}\n\n"
        f"Melhor trade:\n"
        f"{melhor_txt}\n\n"
    ...
    try:
        ...
        if not p.get("tp50_hit") and not p.get("tp50_message_sent"):
            ...
            if tp50_tocado:
                ...
                p["tp50_pnl"] = resultado_tp50

                enviar_tp50(p, tp50, resultado_tp50)
            ...
            if invalidou and int(timestamp_h4) != ultimo_h4_sl100:
                ...
                registrar_fechamento_donkey(
                    ...
                    extra={
                        ...
                        "tp50_pnl": float(p.get("tp50_pnl", pnl_pct(side, entry, float(p.get("tp50", entry))))),
                        "post_tp50_extension_pct": resultado_sl100 - float(p.get("tp50_pnl", pnl_pct(side, entry, float(p.get("tp50", entry))))),
                        "ema20_h4": ema20_h4,
...
def _media(lista, campo):
    valores = [_float0(t.get(campo)) for t in lista if t.get(campo) is not None]
    return sum(valores) / len(valores) if valores else 0.0


def _melhor_txt(lista, campo):
    if not lista:
        return "N/A"
    melhor = max(lista, key=lambda t: _float0(t.get(campo)))
    return f"{melhor.get('symbol_clean', melhor.get('symbol', 'N/A'))} {fmt_pct(_float0(melhor.get(campo)))}"


def calcular_metricas_gestao_dupla_donkey(trades_donkey, data_prefix=None):
    """
    Consolida a gestão dupla Donkey:
    SL50 = 50% fechado pelo trailing curto;
    SL100 = restante fechado pelo candle H4 contra EMA20;
    extensão pós-TP50 = quanto o SL100 adicionou/devolveu depois do TP50.
    """
    if data_prefix:
        base = [t for t in trades_donkey if str(t.get("date", "")).startswith(data_prefix)]
    else:
        base = list(trades_donkey)

    sl50s = [t for t in base if t.get("event") == "SL50"]
    sl100s = [
        t for t in base
        if t.get("event") == "CLOSE"
        and (t.get("closed_by") == "DONKEY_SL100_EMA20_H4" or t.get("sl100_hit") is True)
    ]
    tp50s = [t for t in base if t.get("event") == "TP50"]

    return {
        "tp50s": tp50s,
        "sl50s": sl50s,
        "sl100s": sl100s,
        "media_sl50": _media(sl50s, "pnl"),
        "media_sl100": _media(sl100s, "sl100_pnl"),
        "media_extensao": _media(sl100s, "post_tp50_extension_pct"),
        "media_final_gestao_dupla": _media(sl100s, "pnl"),
        "melhor_extensao": _melhor_txt(sl100s, "post_tp50_extension_pct"),
        "maior_devolucao": _melhor_txt(sl100s, "mfe_gave_back_pct"),
    }


def contar_donkeys_aguardando_sl100():
    try:
        posicoes = carregar_posicoes()
        total = 0
        for p in posicoes.values():
            ...
            if not is_donkey_signal_type(p.get("signal_type")):
                continue
            if bool(p.get("tp50_hit", False)) and bool(p.get("sl50_hit", False)) and not bool(p.get("sl100_hit", False)):
                total += 1
        return total
    except Exception:
        return 0
...
def montar_resumo_donkey():
    ...
    gestao_dupla = calcular_metricas_gestao_dupla_donkey(trades_donkey, hoje)
    aguardando_sl100 = contar_donkeys_aguardando_sl100()
    ...
    linhas = [
        ...
        f"SL50 acionados: {len(gestao_dupla['sl50s'])}",
        f"SL100 acionados: {len(gestao_dupla['sl100s'])}",
        f"Aguardando SL100 após SL50: {aguardando_sl100}",
        "Resultado médio SL50:",
        fmt_pct(gestao_dupla["media_sl50"]),
        "Resultado médio SL100:",
        fmt_pct(gestao_dupla["media_sl100"]),
        "Extensão média pós-TP50:",
        fmt_pct(gestao_dupla["media_extensao"]),
        "Melhor extensão pós-TP50:",
        gestao_dupla["melhor_extensao"],
        "Maior devolução pós-TP50:",
        gestao_dupla["maior_devolucao"],
        "",
        "PnL realizado:",
        fmt_pct(pnl_total),
